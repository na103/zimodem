...
// inizio  funzioni audio ****************************************************************************

// riproduce handshake_audio
void ZCommand::doPlayHandshake()
{
  file = new AudioFileSourcePROGMEM( handshake_audio, sizeof(handshake_audio) );
  wav->begin(file, out);
  
  long starttime = millis();
  long endtime = starttime;
  while ((endtime - starttime) <=10939) 
  {
      if (wav->isRunning()) 
      {
        wav->loop();
...
// riproduce click_audio
void ZCommand::doPlayClick()
{
  file = new AudioFileSourcePROGMEM( click_audio, sizeof(click_audio) );
  wav->begin(file, out);
  
  long starttime = millis();
  long endtime = starttime;
  while ((endtime - starttime) <=254) 
  {
      if (wav->isRunning()) 
      {
        wav->loop();
...
// riproduce busy_audio
void ZCommand::doPlayBusy()
{
  file = new AudioFileSourcePROGMEM( busy_audio, sizeof(busy_audio) );
  wav->begin(file, out);
  
  long starttime = millis();
  long endtime = starttime;
  while ((endtime - starttime) <=4036) 
  {
      if (wav->isRunning()) 
      {
        wav->loop();
...
// riproduce dialing_audio
void ZCommand::doPlayDialing()
{
  file = new AudioFileSourcePROGMEM( dialing_audio, sizeof(dialing_audio) );
  wav->begin(file, out);
  
  long starttime = millis();
  long endtime = starttime;
  while ((endtime - starttime) <=4183) 
  {
      if (wav->isRunning()) 
      {
        wav->loop();
...
void ZCommand::showInitMessage()
{
...
#ifdef INCLUDE_SD_SHELL
  serial.prints("GuruSoundModem WiFi Firmware v");
#else
  ...
  serial.prints(commandMode.EOLN);
  serial.flush();
  // accendo il led Modem Ready collegato al pin 2
  pinMode(pinMR,OUTPUT);
  digitalWrite(pinMR,mrActive);
  // play del click
  doPlayClick();
}
...
void ZCommand::sendConnectionNotice(int id)
{
  ...
  else
  {
    ...
    //a connessione avvenuta play di handshake_audio
    doPlayHandshake();
